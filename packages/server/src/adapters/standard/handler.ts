...
export type StandardHandlerInterceptorOptions<TContext extends Context> = WellStandardHandleOptions<TContext> & { request: LazyStandardRequest }
...
export class StandardHandler<T extends Context> {
  ...
  handle(request: LazyStandardRequest, ...[options]: MaybeOptionalOptions<StandardHandleOptions<T>>): Promise<StandardHandleResult> {
    return intercept(
