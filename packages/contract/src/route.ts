...
export type PrefixRoute<TRoute extends Route, TPrefix extends HTTPPath> = Omit<TRoute, 'path'> & {
  // FIXME: remove the slash `/` from in the beginning of the prefix
  path: TRoute['path'] extends HTTPPath ? `/${TPrefix}${TRoute['path']}` : never
}

export type UnshiftTagRoute<TRoute extends Route, TTag extends string[]> = Omit<TRoute, 'tags'> & {
  tags: TRoute['tags'] extends string[] ? [...TTag, ...TRoute['tags']] : TTag
}
