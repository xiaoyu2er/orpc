...
it('safe', async () => {
  ...
  expect([...r1]).toEqual([null, 1, false, 'success'])
  expect({ ...r1 }).toEqual(expect.objectContaining({ error: null, data: 1, isDefined: false, status: 'success' }))

  const e2 = new Error('error')
  const r2 = await safe(Promise.reject(e2))
  expect([...r2]).toEqual([e2, undefined, false, 'error'])
  expect({ ...r2 }).toEqual(expect.objectContaining({ error: e2, data: undefined, isDefined: false, status: 'error' }))

  const e3 = new ORPCError('BAD_GATEWAY', { defined: true })
  const r3 = await safe(Promise.reject(e3))
  expect([...r3]).toEqual([e3, undefined, true, 'error'])
  expect({ ...r3 }).toEqual(expect.objectContaining({ error: e3, data: undefined, isDefined: true, status: 'error' }))

  const e4 = new ORPCError('BAD_GATEWAY')
  const r4 = await safe(Promise.reject(e4))
  expect([...r4]).toEqual([e4, undefined, false, 'error'])
  expect({ ...r4 }).toEqual(expect.objectContaining({ error: e4, data: undefined, isDefined: false, status: 'error' }))
})
