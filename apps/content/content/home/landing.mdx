...
// safe caller
const gettingSA = createSafeAction({ procedure: getting })
const onSubmit = () => { 
	const [output, error] = await gettingSA({ /***/ })
}
// use with a hook
const { execute, isPending, isError, error, data } = useSafeAction(gettingSA)
