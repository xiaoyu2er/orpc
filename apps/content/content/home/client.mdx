...
const client = createORPCClient<typeof router /* or contract router */>({
  baseURL: 'http://localhost:2026/prefix',
...
const { data: gettingData } = orpc.getting.useQuery({ name: 'dinwwwh' })
...
const data = await utils.getting.fetchQuery({ name: 'dinwwwh' })
...
export function ORPCProvider({ children }: { children: React.ReactNode }) {
    const [client] = useState(() => createORPCClient<typeof router /** must match with createORPCReact*/>({
        baseURL: 'http://localhost:2026/any-prefix',
    }))
...
```bash
curl --request POST \
  --url http://localhost:2026/prefix/getting \
...
```bash
curl --request GET http://localhost:2026/prefix/posts/example-id 
```

```bash
curl -X POST http://localhost:2026/prefix/post/create \
