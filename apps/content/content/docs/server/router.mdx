...
```ts
export const appRouter = ios.router({
    user: {
        find: findUser,
    },
...
```ts
const apiRouter = ios.router({
    users: userRouter,
    posts: postRouter,
    comments: ios.prefix('comments').router(commentRouter),
})
```
...
```ts
const router = ios.prefix('/users').router({
    find: findUserHandler,
    create: createUserHandler,
})
