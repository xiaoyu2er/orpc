...
export type QueryOptionsExtra<TClientContext, TInput, TOutput, TError, TSelectData> =
...
export interface InfiniteOptionsBase<TInput, TOutput, TError> {
  queryKey: QueryKey
...
export type InfiniteOptionsExtra<TClientContext, TInput, TOutput, TError, TSelectData> =
...
export interface MutationOptionsBase<TInput, TOutput, TError> {
  mutationKey: QueryKey
  mutationFn: (input: TInput) => Promise<TOutput>
  retry?: (failureCount: number, error: TError) => boolean // this make tanstack can infer the TError type
}

export type MutationOptionsExtra<TClientContext, TInput, TOutput, TError> =
  & (undefined extends TClientContext ? { context?: TClientContext } : { context: TClientContext })
